...
# Hints
# Hint 1
# Try traversing the BST node by node, all the while keeping track of
# the node with the value closest to the target value. Calculating the
# absolute value of the difference between a node's value and the target
# value should allow you to check if that node is closer than the
# current closest one.
#
# Hint 2
# Make use of the BST property to determine what side of any given node
# has values close to the target value and is therefore worth exploring.
#
# Hint 3
...
# in the BST
# Worst: O(n) time | O(1) space - where n is the number of nodes in
# the BST
    ...
    while node is not None:
        ...
        if value > target:
            if node.left:
                ...
                return min(((v, abs(v - target)) for v in path),
                           key=lambda x: x[1])[0]
        else:
            if node.right:
                node = node.right
