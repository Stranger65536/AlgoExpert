...
# length of 0, while the second subarray should be unsorted. Find
# the smallest (or largest) element in the unsorted subarray and
# insert it into the sorted subarray with a swap. Repeat this process
# of finding the smallest (or largest) element in the unsorted subarray
# and inserting it in its correct position in the sorted subarray with
# a swap until the entire array is sorted.
#
# Optimal Space & Time Complexity
# Best: O(n^2) time | O(1) space - where n is the length of the input
# array
# Average: O(n^2) time | O(1) space - where n is the length of the input
# array
