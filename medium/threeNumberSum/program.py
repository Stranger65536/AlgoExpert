...
#
# Hint 2
# Try sorting the array and traversing it once. At each number, place
# a left pointer on the number immediately to the right of your current
# number and a right pointer on the final number in the array. Check
# if the current number, the left number, and the right number sum up
# to the target sum. How can you proceed from there, remembering the
...
# moving the right pointer one place to the left will lead to a smaller
# right number and thus a smaller sum. This means that, depending on the
# size of each triplet's (current number, left number, right number) sum
